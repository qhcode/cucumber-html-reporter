  ...
  const setStats = function (suite) {
    ...
    featureOutput.forEach(function (feature) {
      ...
      feature.elements.forEach(function (element) {
        ...
        function getEmbeddedImageExtension(type) {
          let imageExtension = type.replace("image/","")
        ...
        element.steps.forEach(function (step, count) {
          if (step.embeddings !== undefined) {
            step.embeddings.forEach(function (embedding) {
              ...
              if (['text/plain', 'text/html', 'application/json'].includes(embeddingType)) {
              ...
              } else if (['image/apng', 'image/avif', 'image/gif', 'image/jpeg', 'image/png','image/svg+xml','image/webp'].includes(embeddingType)) {
                step.image = `data:${embeddingType};base64,` + embedding.data;
                ...
                if (options.storeScreenshots && options.storeScreenshots === true) {
                  ...
                  name = name + '_' + Math.round(Math.random() * 10000) + getEmbeddedImageExtension(embeddingType); //randomize the file name
